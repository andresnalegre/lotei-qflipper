...
static const int   NIKITA_MAX_TOOL_ROUNDS = 12;   // more headroom for multi-step agent work
...
// known to behave well with NIKITA's tool-calling. Order here is also the
...
// and captured output -- enough for NIKITA to build, run tests and fix bugs in
...
// log say NIKITA made the app speak with two voices about the same thing.
...
// user asks and NIKITA silently CALLS save_file (empty content + a tool_calls
    ...
    : QObject(parent)
{
    ...
    m_manualName = QSettings().value(QStringLiteral("nikita/manualName")).toString();
...
QString NikitaBackend::manualName() const { return m_manualName; }

void NikitaBackend::setManualName(const QString &name)
{
    if (name == m_manualName) { return; }
    m_manualName = name;
    QSettings().setValue(QStringLiteral("nikita/manualName"), m_manualName);
...
QString NikitaBackend::systemPrompt() const
{
    ...
    if (!name.isEmpty()) {
        sys += QStringLiteral("\n\nYOUR NAME -- IMPORTANT: you are bonded to a Flipper Zero named "
            "\"%1\", so your name is %1 (NOT NIKITA -- that's just your underlying model line). "
...
void NikitaBackend::finalizeStream()
{
    ...
    if (!toolCalls.isEmpty() && m_toolRounds < NIKITA_MAX_TOOL_ROUNDS) {
...
void NikitaBackend::runOneTool(const QString &name, const QJsonObject &args, std::function<void(const QString &)> done)
{
    ...
    if (kLoggedTools.contains(name) || isHost) {
        ...
        const QJsonObject learnArgs = args;
            ...
            if (nikitaResultProves(result)) { recordProvenMove(name, learnArgs); }
                               ...
                               std::function<void(const QString &)> done)
{
    ...
    if (name == QLatin1String("host_list")) {
        ...
        const bool finished = proc.waitForFinished(NIKITA_HOST_RUN_TIMEOUT_MS);
        if (!finished) {
            proc.kill();
        ...
        bool truncated = out.size() > NIKITA_HOST_OUTPUT_CAP;
...
bool NikitaBackend::agentEnabled() const { return m_agentEnabled; }QString NikitaBackend::agentDir() const  { return m_agentRoot; }
...
// memory to the Flipper SD at /ext/nikita/memory.txt when a device is around.
